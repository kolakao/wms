...
$grandreset_dl_points     = trim($load_reset_settings->grandreset_dl_points);
$grandreset_mg_points     = trim($load_reset_settings->grandreset_mg_points);
    ...
    if(empty($id) || !is_numeric($id)){
        ...
        if(character_and_account($id,$user_auth_id) === false){
            ...
            if(account_online($user_auth_id) === true){
                ...
                $select_req = $core_db->Execute("select clevel,money,resets,leveluppoint,grand_resets,class from character where mu_id=? and accountid=?",array($id,$user_auth_id));
                ...
                if($no_reset != '1'){
                    ...
                    switch ($reset_points_formula){
                        case '0': $new_bpoints = ($reset_points); break;
                        case '1':
								if($select_req->fields[5] == $mg_class ){
									$new_bpoints = ($grandreset_mg_points*($select_req->fields[4]+1)); 
								}
								elseif($select_req->fields[5] == $dl_class ){
									$new_bpoints = ($grandreset_dl_points*($select_req->fields[4]+1)); 
								}
								else{
									$new_bpoints = ($reset_points * ($select_req->fields[4]+1)); 
								}
							break;
                    }
