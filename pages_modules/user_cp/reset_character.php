...
if ($active == '0') {
    ...
    $grand_reset_points   = trim($load_grand_reset_settings->bpoints);
    ...
    $reset_mg_points     = trim($load_reset_settings->reset_mg_points);
    $reset_dl_points     = trim($load_reset_settings->reset_dl_points);
	$grandreset_dl_points     = trim($load_grand_reset_settings->grandreset_dl_points);
	$grandreset_mg_points     = trim($load_grand_reset_settings->grandreset_mg_points);
    $dl_class 		  = trim($load_reset_settings->dl_class);
    $mg_class 		  = trim($load_reset_settings->mg_class);
    if (isset($_GET['rid'])) {
        ...
        if (empty($id) || !is_numeric($id)) {
            ...
            if (character_and_account($id, $user_auth_id) === false) {
                ...
                if (account_online($user_auth_id) === true) {
                    ...
                    $select_req = $core_db->Execute("select clevel,money,resets,leveluppoint,grand_resets,class from character where mu_id=? and accountid=?", array(
                        $id,
                        $user_auth_id
                    ));
                    ...
                    if ($no_reset != '1') {
                        ...
                        switch ($reset_points_formula) {
                            case '0':
                                ...
                                break;
                            case '1':
								if($select_req-> fields[5] == $dl_class){
									$new_bpoints = (($reset_dl_points * ($select_req->fields[2] + 1)) + ($grandreset_dl_points * $select_req->fields[4]));
								}
								elseif($select_req-> fields[5] == $mg_class){
									$new_bpoints = (($reset_mg_points * ($select_req->fields[2] + 1)) + ($grandreset_mg_points * $select_req->fields[4]));
								}
								else{	
									$new_bpoints = (($reset_points * ($select_req->fields[2] + 1)) + ($grand_reset_points * $select_req->fields[4]));
								}
                                break;
                        }
                        switch ($reset_stats) {
